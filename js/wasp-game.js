...
const createWasps = (waspsToCreate = []) => {
    if (waspsToCreate.length == 0) return false;
    
    let createdWasps = '';
    waspsToCreate.map((waspDefinition) => {
        for (let i = 0; i < waspDefinition.qty; i++) {
            createdWasps = createdWasps + `${waspDefinition.type}:${waspDefinition.hitPoints}:${waspDefinition.damage}:${waspDefinition.hitPoints},`;
        }
    });
    return createdWasps.slice(0, -1);
}

const updateCurrentPoints = (selectedWasp) => {
    let waspDetails = selectedWasp.split(':');
    let [ type, hitPoints, damage, currentPoints ] = waspDetails;
    let newPoints = currentPoints - damage;
    if (newPoints < 0) newPoints = 0;
    return newPoints;
}

const hitWasp = (e, waspList = null) => {
    if (!waspList) waspList = document.getElementById('wasp-list').textContent;
    let currentSwarm = waspList.split(',');
    let waspToHit = null;
    ...
    let updatedWasp = currentSwarm[waspToHit].split(':');
    updatedWasp[3] = newPoints;
    currentSwarm[waspToHit] = updatedWasp.join(':');

    if (e) {
        renderWaspGame(`#${e.target.parentElement.id}`, 'replaceParent', currentSwarm.join(','));
    }

    if (parseInt(updatedWasp[3]) === 0 && updatedWasp[0] === 'queen') {
        return endGame(e);
    }

    return (currentSwarm.join(','));
}
...
function renderWaspGame(targetElement = 'body', insertMethod = 'replace', wasps = null) {
    const el = document.querySelector(targetElement);

    if (!wasps) wasps = createWasps(config);
    const buttonHtml = `<button id="hitWasp">
    ...
    const gameTemplate = `<div id="wasp-game-container"><div id="wasp-list">${wasps}</div>${buttonHtml}</div>`;

    try {
        if (insertMethod === 'replace') el.innerHTML = gameTemplate;
        if (insertMethod === 'replaceParent') el.parentElement.innerHTML = gameTemplate;
        if (insertMethod === 'append') {
            let newEl = document.createElement('div');
            newEl.innerHTML = gameTemplate;
            el.appendChild(newEl);
        }
        if (document.location.search.includes('style=true')) {
            addSomeStyle();
        }
        try {
            document.querySelector('#hitWasp').addEventListener('click', hitWasp);
        } catch(e) {
            console.error('Unable to add click event, button not found')
        }
    } catch(e) {
...
function addSomeStyle() {
    const swarm = document.getElementById('wasp-list').textContent;
    let newList = document.createElement('ul');
    newList.setAttribute('id', 'wasp-list-styled');

    swarm.split(',').map((wasp) => { 
        const waspDetails = wasp.split(':');
        const [ type, hitPoints, damage, currentPoints ] = waspDetails;
        
        let newMeter = document.createElement('meter');
        newMeter.setAttribute('min', 0);
        newMeter.setAttribute('max', hitPoints);
        newMeter.setAttribute('value', currentPoints);
        newMeter.setAttribute('low', (parseInt(damage) + 1));
        newMeter.setAttribute('high', (parseInt(hitPoints) - parseInt(damage)));
        newMeter.setAttribute('optimum', hitPoints);
        
        let newListEl = document.createElement('li');
        newListEl.setAttribute('data-type', type);
        newListEl.setAttribute('data-damage', damage);
        if (currentPoints == 0) newListEl.setAttribute('class', 'dead');

        newListEl.appendChild(newMeter);
        newList.appendChild(newListEl);
    });

    document.getElementById('wasp-game-container').insertBefore(
        newList,
        document.getElementById('hitWasp')
    );
}